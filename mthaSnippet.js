...
if (typeof matterhornKeeper === 'undefined') {
    var matterhornKeeper = {"pagename": window.location.pathname}
}

var n_matterhorn_vid = false;

(function(){
    if(sessionStorage.getItem("tab") === null){
        var matterhorn_visit_id = new Date().getTime();
        sessionStorage.setItem('tab', matterhorn_visit_id);
        sessionStorage.setItem('o', document.referrer);
        var matterhorn_url_params = new URL(window.location.href).searchParams;
        sessionStorage.setItem('sr', matterhorn_url_params.get("utm_source"));
        sessionStorage.setItem('md', matterhorn_url_params.get("utm_medium"));
        sessionStorage.setItem('ca', matterhorn_url_params.get("utm_campaign"));
    }else{
        var matterhorn_visit_id = sessionStorage.getItem("tab")
    }
    var matterhorn_vid = localStorage.getItem('matterhorn_vid');
    var n_matterhorn_vid = String(matterhorn_vid) == "null" || matterhorn_vid.length < 3;
    matterhorn_vid = n_matterhorn_vid ? String(new Date().getTime()) + "." + String(Math.floor(Math.random() * 1000000)) : matterhorn_vid;
    if(n_matterhorn_vid){
        localStorage.setItem('matterhorn_vid', matterhorn_vid);
    }
})();

function matterhorn_send(obj){
    console.log(obj)
	headers = { 'Content-Type': 'application/json' };
	var xhr = new XMLHttpRequest();
	var url = "https://25db-177-60-73-11.sa.ngrok.io/mhc";
...
function matterhorn_event(e, ee){
	...
	var eventObj = 
		{
            ...
            "mid": new DeviceUUID().get(),
			"vid": localStorage.getItem('matterhorn_vid'),
			"uid": matterhornKeeper["uid"] ? matterhornKeeper["uid"] : "null",
			"fv": n_matterhorn_vid,
            "url": window.location.href,
            "hn": window.location.hostname,
            "t": document.title,
            "pn": matterhornKeeper["pagename"].trim().length > 0 ? matterhornKeeper["pagename"] : window.location.pathname,
            "sr": sessionStorage.getItem("sr"),
            "md": sessionStorage.getItem("md"),
            "ca": sessionStorage.getItem("ca"),
			"o": sessionStorage.getItem("o"),
			"dv": device,
            "os": du["os"],
            "br": du["browser"],
			"l": du["language"],
            ...
            "m": d.getMonth()+1,
            "d": d.getDate(),
			"dt": [d.getFullYear(),d.getMonth()+1,d.getDate()].join('-'),
            "h": d.getHours(),
            "mi": d.getMinutes(),
            "e": e,
            "ee": ee
		};
    matterhorn_send(eventObj)
}
